...
function fixOldFkTable(db, name, createSql) {
  ...
  if (common.length) {
    db.exec(
      `INSERT INTO ${name} (${common.join(",")}) SELECT ${common.join(",")} FROM ${tmp};`
    );
  }
  ...
  try {
    if (name === "EDICION") {
      ...
      db.exec(`CREATE INDEX IF NOT EXISTS idx_edicion_editorial ON EDICION (EDITORIAL_ID);`);
    }

    if (name === "LIBRO_AUTOR") {
      db.exec(`CREATE INDEX IF NOT EXISTS idx_libro_autor_autor ON LIBRO_AUTOR (AUTOR_ID);`);
    }
    ...
    if (name === "RESERVA") {
      ...
      db.exec(`CREATE UNIQUE INDEX IF NOT EXISTS uq_reserva_activa_usuario_libro ON RESERVA (USUARIO_ID, LIBRO_ID) WHERE ESTADO = 'activa';`);
    }
  } catch(e) {}
}
...
function createLibroAutorSql() {
  return `
    CREATE TABLE LIBRO_AUTOR (
      ...
      FOREIGN KEY (LIBRO_ID) REFERENCES LIBRO(ID) ON UPDATE CASCADE ON DELETE CASCADE,
      FOREIGN KEY (AUTOR_ID) REFERENCES AUTOR(ID) ON UPDATE CASCADE ON DELETE RESTRICT
    ) WITHOUT ROWID;
  `;
}
...
function createReservaSql() {
  return `
    CREATE TABLE RESERVA (
      ...
      EXPIRA_EN     TEXT,
      ESTADO        TEXT NOT NULL DEFAULT 'activa' CHECK (ESTADO IN ('activa','cancelada','cumplida','expirada')),
      FOREIGN KEY (USUARIO_ID) REFERENCES USUARIO(ID) ON UPDATE CASCADE ON DELETE RESTRICT,
      FOREIGN KEY (LIBRO_ID) REFERENCES LIBRO(ID) ON UPDATE CASCADE ON DELETE RESTRICT
    );
  `;
}
