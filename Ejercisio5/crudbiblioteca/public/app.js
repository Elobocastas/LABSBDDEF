...
async function openAutoresAsignacion(libroId) {
  ...
  const wrap = h("div", {}, [
    h("div", { class: "muted", style: "margin-bottom:8px;" }, ["Selecciona autores para este libro."]),
  ]);
...
async function newUsuario() {
  const form = h("div", { class: "form" }, [
    ...
    field("Teléfono", "telefono", ""),
    field("Dirección", "direccion", "", true),
    field("Tipo (alumno/docente/externo/bibliotecario)", "tipo", "alumno"),
...
async function newEjemplar() {
  ...
  const form = h("div", { class: "form" }, [
    ...
    field("Estado (disponible/prestado/mantenimiento)", "estado", "disponible")
  ]);
...
async function newPrestamo() {
  ...
  const save = async () => {
    ...
    await api.send("/api/prestamos", "POST", {
      usuario_id,
      observaciones,
      items: items.map((x) => ({ ejemplar_id: x.ejemplar_id }))
    });
